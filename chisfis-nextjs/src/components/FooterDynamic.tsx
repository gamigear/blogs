...
const FooterDynamic: React.FC = () => {
  ...
  // Convert footerLinks to menu format - only from database
  const menus = Object.entries(footerLinks).map(([section, links]) => ({
    id: section,
    ...
    <div className="relative border-t border-neutral-200 py-24 lg:py-28 dark:border-neutral-700">
      <div className="container grid grid-cols-2 gap-x-5 gap-y-10 sm:gap-x-8 md:grid-cols-4 lg:grid-cols-5 lg:gap-x-10">
        <div className="col-span-2 grid grid-cols-4 gap-5 md:col-span-4 lg:flex lg:flex-col lg:md:col-span-1">
          ...
          {socialLinks.length > 0 && (
            <div className="col-span-2 flex items-center md:col-span-3">
              <div className="flex items-center gap-x-3 lg:flex-col lg:items-start lg:gap-x-0 lg:gap-y-2.5">
                {socialLinks.map((social, index) => (
                  <a
                    ...
                    className="flex items-center gap-2 text-neutral-600 hover:text-black dark:text-neutral-300 dark:hover:text-white"
                  >
                    {getSocialIcon(social.platform)}
                    <span className="hidden lg:inline capitalize">{social.platform}</span>
                  </a>
                ))}
              </div>
            </div>
        ...
        {menus.map((menu, index) => (
          <div key={menu.id || index} className="text-sm">
      ...
      {settings?.footer?.copyright && (
        <div className="container mt-10 pt-6 border-t border-neutral-200 dark:border-neutral-700">
