...
const PostBlack = ({ post, color, style }) => {
  ...
  return (
    <article className="post-card post-category-top group relative has-line-link-white">
      <div className="relative">
        <span className={`post-category text-white bg-${color} z-10`}>
          <Link
            href={`/category/${slugify(category)}`}
            className={`transition duration-300 bg-${color} text-white border-white/30 hover:bg-white hover:text-dark`}
          >
            {category}
          </Link>

          <div className={`text-${color} corner left`}>
          ...
          <div className={`text-${color} corner bottom`}>
            <svg width="101" height="101" viewBox="0 0 101 101" fill="none" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" clipRule="evenodd" d="M101 0H0V101H1C1 45.7715 45.7715 1 101 1V0Z" fill="currentColor"></path></svg>
          </div>
        </span>
        <Image
      ...
      >
        <span
          className={`text-sm flex gap-2 items-center mb-3 uppercase ${
