(function () {
  ...
  var calendar = new FullCalendar.Calendar(document.getElementById('calendar'), {
   ...
   // Handle click on a date in the calendar
    dateClick: function (info) {
      ...
      // Set the clicked date in the 'From' date input
      // Use moment.js to format the date to "DD MMMM, YYYY"
      document.getElementById('fromDate').value = moment(info.date).format('DD MMMM, YYYY');
  ...
  document.getElementById('addEventButton').addEventListener('click', function () {
      ...
      // Convert flatpickr dates to YYYY-MM-DD format
      var fromDate = moment(fromDateStr).format("YYYY-MM-DD");
      var toDate = moment(toDateStr).format("YYYY-MM-DD");

      // Add one day to the 'To' date to make the range inclusive
      var adjustedToDate = moment(toDate).add(1, 'days').format("YYYY-MM-DD");
      ...
      // Safely clear the TomSelect component
      var tomSelectElement = document.querySelector('#addEvent .tom-select');
