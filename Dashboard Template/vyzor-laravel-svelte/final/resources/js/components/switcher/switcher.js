...
const createUserStore = () => {
    ...
    return {
        ...
        updateColorThemeFn(newTheme) {
            ...
            userData.update((data) => {
                ...
                if (newTheme === 'dark') {
                    ...
                    updatedState.headerColor = localStorage.getItem('vyzorHeader') || 'transparent';
                } else {
        ...
        updateNavigationStylesFn(value) {
            ...
            const menuColor = localStorage.getItem('vyzorMenu') ? localStorage.getItem('vyzorMenu') : "transparent"
            const themeDark = localStorage.getItem('vyzorMenu') ? localStorage.getItem('vyzorMenu') : "transparent"
            if (value === 'horizontal') {
        ...
        updateLayoutStylesFn(value) {
            ...
            if (value === 'double-menu') {
                ...
                tooltip.style.setProperty("background-color", "var(--default-body-bg-color)");
                tooltip.style.setProperty("color", "var(--text-default)");
        ...
        retrieveUserLocalStorage() {
            const savedTheme = localStorage.getItem('vyzorcolortheme') || 'light';
            if (savedTheme) this.updateColorThemeFn(savedTheme);

            const savedDirection = localStorage.getItem('vyzordirection');
            if (savedDirection) this.UpdateDirectionFn(savedDirection);

            const savedNavigationStyles = localStorage.getItem('vyzornavstyles') || 'vertical';
            if (savedNavigationStyles) this.updateNavigationStylesFn(savedNavigationStyles);

            const savedPages = localStorage.getItem("vyzorpageStyle");
            if (savedPages) this.updatePageStylesFn(savedPages);

            const savedwidthStyles = localStorage.getItem("vyzorwidthStyles");
            if (savedwidthStyles) this.updateWidthStylesFn(savedwidthStyles);

            const savedMenuPosition = localStorage.getItem("vyzormenuposition");
            if (savedMenuPosition) this.updateMenuPositionFn(savedMenuPosition);

            const savedHeaderPosition = localStorage.getItem("vyzorheaderposition");
            if (savedHeaderPosition) this.updateHeaderPositionFn(savedHeaderPosition);

            const savedThemePrimary = localStorage.getItem("vyzorprimaryRGB");
            if (savedThemePrimary) this.updateThemePrimaryFn(savedThemePrimary);

            const savedBackgroundImage = localStorage.getItem("vyzorbgimg");
            ...
            const savedLightBackground = localStorage.getItem("vyzorbodylightRGB");
            ...
            const savedMenuColors = localStorage.getItem('vyzorMenu') ? localStorage.getItem('vyzorMenu') :
                (savedTheme === 'dark' ? 'dark' : localStorage.getItem('vyzorMenu'));
            if (savedMenuColors) this.updateMenuColorFn(savedMenuColors);

            let savedHeaderColors = localStorage.getItem('vyzorHeader') ? localStorage.getItem('vyzorHeader') :
                (savedTheme === 'dark' ? 'transparent' : localStorage.getItem('vyzorHeader')); // Similarly for header colors
            if (savedHeaderColors) this.updateHeaderColorFn(savedHeaderColors);

            const savedMenuStyles = localStorage.getItem("vyzormenuStyles");
            if (!localStorage.getItem('vyzorverticalstyles')) {
                this.updateMenuStylesFn(savedMenuStyles);
            }

            const savedLayoutStyles = localStorage.getItem("vyzorverticalstyles");
        ...
        custompageLocalStorage() {
            ...
            let savedColorTheme = localStorage.getItem('vyzorcolortheme') || 'light';
            if (savedColorTheme) this.updateColorThemeFn(savedColorTheme);
            let savedDirection = localStorage.getItem('vyzordirection')
            if (savedDirection) this.UpdateDirectionFn(savedDirection);
            let savedNavigationStyle = 'horizontal'
            if (savedNavigationStyle) this.updateNavigationStylesFn(savedNavigationStyle);
            let savedThemePrimary = localStorage.getItem("vyzorprimaryRGB");
            if (savedThemePrimary) this.updateThemePrimaryFn(savedThemePrimary);
