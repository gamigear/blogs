...
import { SpkInvoicelistCard } from '../../../../@spk/reusable-pages/spk-invoicelist-card/spk-invoicelist-card';
...
@Component({
  ...
  standalone: true,
  imports: [SharedModule,NgbDropdownModule,SpkReusableTables,SpkInvoicelistCard,SpkDropdowns,RouterModule],
...
remove(id:string){
	const data = this.Invoices.filter((x: { id: string }) => x.id !== id);
	this.Invoices = data;
  }
