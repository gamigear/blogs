...
const Menuloop = ({ MenuItems, level, handleToMenu, HoverToggleInnerMenuFn }) => {
  ...
  return (
    <Fragment>
        <BaseLink href="#!"  className={`side-menu__item ${MenuItems?.selected ? "active" : ""}`}
      ...
      >
        {level <= 1 ? <li className="slide side-menu__label1">
          <BaseLink href="#!" >{MenuItems.title}  </BaseLink>
        </li> : ""}
        {MenuItems?.children.map((firstlevel, index) =>
          ...
          >
            ...
            {firstlevel?.type === "empty" ?
              <BaseLink href="#!" className={`side-menu__item`} onClick={handleClick}>
                {firstlevel.icon}
                {firstlevel.title}
              </BaseLink>
              : ""}
