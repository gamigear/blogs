            ...
            <Col xxl={12} xl={12} className="">
                <Card className="custom-card">
                    <Card.Header className="justify-content-between">
                        ...
                        <div className="d-flex custom-task">
                                        ...
                                        <Col xl={6}>
                                            ...
                                            <div className="form-group">
                                                <div className="input-group flex-nowrap">
                                                    ...
                                                    <SpkDatepickr className="form-control" Timeinput="Time:" dateFormat="yy/MM/dd h:mm aa" selected={dates["startDate"] ? new Date(dates["startDate"]) : null} onChange={(date) => handleDateChange("startDate", date)} placeholderText='Choose date and time' showTimeInput />
                                                </div>
                                            </div>
                                        </Col>
                                        <Col xl={6}>
                                            ...
                                            <div className="form-group">
                                                <div className="input-group flex-nowrap">
                                                    <div className="input-group-text text-muted"> <i className="ri-calendar-line"></i> </div>
                                                    <SpkDatepickr className="form-control" Timeinput="Time:" dateFormat="yy/MM/dd h:mm aa" selected={dates["dueDate"] ? new Date(dates["dueDate"]) : null} onChange={(date) => handleDateChange("dueDate", date)} placeholderText='Choose date and time' showTimeInput />
                                                </div>
                                            </div>
                                        </Col>
                                        <Col xl={6}>
                            ...
                            <SpkDropdown toggleas="a"  Customtoggleclass="btn btn-icon btn-secondary-light btn-sm btn-wave waves-light no-caret" IconClass="ti ti-dots-vertical" Icon={true}>
                    ...
                    <Card.Body className="p-0">
                        <div className="table-responsive">
                            <SpkTables tableClass="text-nowrap" headerClass="table-light" checked={selectedTasks.every((selected) => selected)} onChange={handleSelectAll} showCheckbox={true} header={[{ title: 'Task' }, { title: 'Task ID' }, { title: 'Assigned Date' }, { title: 'Status' }, { title: 'Due Date' }, { title: 'Priority' }, { title: 'Assigned To' }, { title: 'Action' }]} >
                                {allData.map((task, index) => (
                                    <tr className="task-list" key={task.id}>
                                        <td className="task-checkbox">
                                            <input className="form-check-input" type="checkbox" aria-label="..." checked={selectedTasks[index]} onChange={() => handleCheckboxChange(index)} />
                                        </td>
                                        <td>
                                            <BaseLink href="#!" className="fw-medium" data-bs-toggle="offcanvas" data-bs-target="#offcanvasRight" aria-controls="offcanvasRight">
                                                {task.title}
                                            </BaseLink>
                                        </td>
...
// Assign layout
TaskListView.layout = (page) => <MainLayout>{page}</MainLayout>;
