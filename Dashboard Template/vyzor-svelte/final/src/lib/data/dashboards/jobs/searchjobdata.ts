 ...
 * @param {HTMLElement} lowerValueNode - The DOM element displaying the lower value
 * @param {HTMLElement} upperValueNode - The DOM element displaying the upper value
 * @param {function} updateValues - Callback to update the lower and upper values
 */
export function initializeSlider(sliderElement, lowerValueNode, upperValueNode, updateValues) {
  ...
  // Update the displayed values as the slider is used
  sliderElement.noUiSlider.on('update', function (values, handle) {
    if (handle === 0) {
      const lowerValue = parseInt(values[0]);
      lowerValueNode.innerHTML = lowerValue;
      updateValues(lowerValue, null); // Update only the lower value
    } else {
      const upperValue = parseInt(values[1]);
      upperValueNode.innerHTML = upperValue;
      updateValues(null, upperValue); // Update only the upper value
    }
  });
}
...
export const JobTable: Job[] = [
  {
    ...
    shiftColor:'bg-primary-transparent'
  },
  {
    id: 2,
    title: "Data Analyst",
  ...
  },
  {
    ...
    shiftColor:'bg-warning-transparent'
  },
  {
    id: 4,
  ...
  },
  {
    ...
    featured: false,
    shiftColor:'bg-success-transparent'
  },
];