<script lang="ts">
    ...
    import {
        ...
        Drag8,
        Drag9,
    } from "$lib/data/adavanec-ui/sortabledata";
    import {
        Card,
        CardBody,
        CardHeader,
    ...
    let MultiDrag =
        (Sortable as any).MultiDrag || (SortablePkg as any).MultiDrag;

    if (MultiDrag) {
        Sortable.mount(new MultiDrag());
    }

    let swap = (Sortable as any).Swap || (SortablePkg as any).Swap;
    if (swap) {
        Sortable.mount(new swap());
    }

    let listContainer: HTMLUListElement;
    let containerA: HTMLUListElement;
    let containerB: HTMLUListElement;
    let cloningA: HTMLUListElement;
    let cloningB: HTMLUListElement;
    let disableA: HTMLUListElement;
    let disableB: HTMLUListElement;
    let HandleDrag: HTMLUListElement;
    let filterDrag: HTMLUListElement;
    let sortableGrid: any;
    let multiDrag: HTMLUListElement;
    let swapDrag: HTMLUListElement;
    onMount(() => {
    ...
    let level1Els: { [id: string]: HTMLDivElement } = {};
    let level2Els: { [id: string]: HTMLDivElement } = {};

    function reorder(list: ListItem[], from: number, to: number) {
        const item = list.splice(from, 1)[0];
        list.splice(to, 0, item);
    }

    function setupSortable(list: ListItem[], id: string) {
