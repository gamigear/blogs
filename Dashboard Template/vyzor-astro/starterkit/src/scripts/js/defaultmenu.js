...
export function setNavActive() {
  const basePath = import.meta.env.BASE_URL; // "/" or "/astro/vyzor/preview/"
  let currentPath = window.location.pathname;

  // Remove basePath from currentPath
  if (currentPath.startsWith(basePath)) {
    currentPath = currentPath.slice(basePath.length - 1); // keep leading "/"
  }
  ...
  sidemenuItems.forEach((item) => {
    ...
    if (href.startsWith(basePath)) href = href.slice(basePath.length - 1);
    ...
    if (currentPath === href || currentPath.startsWith(href + "/")) {
      ...
      item.parentElement.classList.add("active", "open");

      // Open parent menus
      let parentUl = item.closest("ul");
      while (parentUl) {
        const prev = parentUl.previousElementSibling;
        if (prev) prev.classList.add("active", "open");
        parentUl.parentElement?.classList.add("active", "open");

        const childUl = parentUl.querySelector("li.active > ul");
