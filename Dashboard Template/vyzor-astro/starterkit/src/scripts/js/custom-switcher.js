...
import {
    ResizeMenu, toggleSidemenu, closedSidemenuFn, detachedFn, iconTextFn, doubletFn, menuClickFn, menuhoverFn, iconClickFn, iconHoverFn, setNavActive, clearNavDropdown,
    checkHoriMenu, iconOverayFn
} from './defaultmenu.js'
...
function ltrFn() {
    ...
    // Get the style element safely
    let styleEl = document.querySelector("#style");

    if (styleEl) {
        // Only access href if the element exists
        if (!styleEl.href.includes('bootstrap.min.css')) {
            styleEl.setAttribute("href", "../scripts/libs/bootstrap/css/bootstrap.min.css");
        }
    }
...
function rtlFn() {
    ...
    document.querySelector("#style")?.setAttribute("href", "../scripts/libs/bootstrap/css/bootstrap.rtl.min.css");
    checkOptions();
}
