...
export default function EditCustomScriptPage() {
  ...
  const fetchScript = async () => {
    try {
      const res = await fetch(`/api/admin/custom-scripts/${params.id}`);
      if (res.ok) {
        ...
        setError('Không tìm thấy script');
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Có lỗi xảy ra khi cập nhật script');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Chỉnh sửa Script</h1>
          <p className="text-gray-500 dark:text-gray-400">Cập nhật custom code</p>
        </div>
      </div>
