...
export default function SettingsPage() {
  ...
  const [mediaPickerField, setMediaPickerField] = useState<'logo_header' | 'logo_footer' | 'favicon' | 'default_avatar' | 'default_cover' | null>(null);
  ...
  const defaultSettings: Settings = {
    general: { site_name: '', site_description: '', logo_header_url: '', logo_footer_url: '', favicon_url: '', default_avatar: '', default_cover: '', contact_email: '', contact_phone: '' },
      ...
      <div className="rounded-md bg-white dark:bg-[#1A1D1F] p-6">
        ...
        {activeTab === 'general' && settings && (
          <div className="space-y-5">
            ...
            {/* Default Avatar & Cover Section */}
            <div className="border-t border-gray-200 dark:border-gray-700 pt-5 mt-5">
              <h4 className="font-semibold text-gray-900 dark:text-white mb-4">Avatar & Cover mặc định</h4>
              <p className="text-sm text-gray-500 mb-4">Hình ảnh này sẽ được sử dụng cho người dùng chưa có avatar/cover riêng</p>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Default Avatar */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Avatar mặc định</label>
                  <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-md p-4 text-center">
                ...
                {/* Default Cover */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Cover mặc định</label>
                  <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-md p-4 text-center">
                    {settings.general?.default_cover ? (
                      <div className="space-y-3">
                        <img src={settings.general.default_cover} alt="Default Cover" className="w-full h-20 mx-auto object-cover rounded" />
                        <div className="flex gap-2 justify-center">
                          <button type="button" onClick={() => setMediaPickerField('default_cover')}
                            className="px-3 py-1.5 text-xs bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded hover:bg-gray-200 dark:hover:bg-gray-600">
                            Thay đổi
                          </button>
                          <button type="button" onClick={() => updateSetting('general', 'default_cover', '')}
                            className="px-3 py-1.5 text-xs bg-red-100 text-red-600 rounded hover:bg-red-200">
                            Xóa
                          </button>
                        </div>
                      ...
                      <button type="button" onClick={() => setMediaPickerField('default_cover')}
                        ...
                        <span className="text-sm">Chọn cover mặc định</span>
                      </button>
                    )}
                  </div>
                  <p className="text-xs text-gray-500 mt-1">Khuyến nghị: 1200x400 pixels hoặc tỷ lệ 3:1</p>
                </div>
              </div>
            </div>
          </div>
